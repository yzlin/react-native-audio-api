...
- (void)setValue:(float)value
{
  if (value < _minValue || value > _maxValue) {
    ...
    @throw [NSException exceptionWithName:NSInternalInconsistencyException
                                   reason:[NSString stringWithFormat:@"Value out of bounds"]
                                 userInfo:nil];
  }
...
- (double)calculateSetValueAtTime:(double)startTime
...
{
  ParamChange *param =
      [[ParamChange alloc] init:[self getStartTime]
                        endTime:time
                     startValue:[self getStartValue]
                       endValue:value
                         target:self
              calculateSelector:@selector(calculateLinearValueWithStartTime:endTime:startValue:endValue:currentTime:)];
  [_timeline insert:param];
...
- (double)calculateLinearValueWithStartTime:(double)startTime
                                    endTime:(double)endTime
                                 startValue:(double)startValue
                                   endValue:(double)endValue
                                currentTime:(double)currentTime
...
{
  ParamChange *param = [[ParamChange alloc]
                   init:[self getStartTime]
                endTime:time
             startValue:[self getStartValue]
               endValue:value
                 target:self
      calculateSelector:@selector(calculateExponentialRampToValueAtTime:endTime:startValue:endValue:currentTime:)];
  [_timeline insert:param];
}

- (double)calculateExponentialRampToValueAtTime:(double)startTime
                                        endTime:(double)endTime
                                     startValue:(double)startValue
                                       endValue:(double)endValue
                                    currentTime:(double)currentTime
