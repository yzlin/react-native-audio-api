...
+ (FilterTypeEnum)filterTypeFromString:(NSString *)type;
{
  ...
  @throw [NSException
      exceptionWithName:@"Invalid filter type"
                 reason:
                     @"You have to pick correct wave type: ['lowpass', 'highpass', 'bandpass', 'lowshelf', 'highshelf', 'peaking', 'notch', 'allpass']"
               userInfo:nil];
}
...
+ (NSString *)stringFromFilterType:(FilterTypeEnum)filterType
{
  switch (filterType) {
    ...
    default:
      @throw [NSException
          exceptionWithName:@"Invalid wave type"
                     reason:
                         @"You have to pick correct wave type: ['lowpass', 'highpass', 'bandpass', 'lowshelf', 'highshelf', 'peaking', 'notch', 'allpass']"
                   userInfo:nil];
  }
