...
#include <audioapi/core/v2/AudioParam.h>
...
void AudioParam::processParamNoInput(
    const std::shared_ptr<audioapi::AudioBus> &outputBus,
    int framesToProcess,
    double time,
    float sampleRate) {
...
void AudioParam::processParam(
  ...
  if (inputNodes_.empty()) {
    ...
    // when all inputs are disabled, processingBus will be nullptr
...
std::shared_ptr<audioapi::AudioBus> AudioParam::processInputs(
    const std::shared_ptr<audioapi::AudioBus> &outputBus,
    int framesToProcess,
    bool checkIsAlreadyProcessed) {
  std::shared_ptr<audioapi::AudioBus> processingBus = nullptr;
  ...
  for (auto it = inputNodes_.begin(), end = inputNodes_.end(); it != end;
    ...
    } else if (maxNumberOfChannels < inputBus->getNumberOfChannels()) {
      maxNumberOfChannels = inputBus->getNumberOfChannels();
      processingBus = inputBus;
    }
  }
