...
We have used the [`decodeAudioDataSource`](/core/base-audio-context#decodeaudiodatasource) method of the [`BaseAudioContext`](/core/base-audio-context), which takes a URL to a local file or bundled audio asset and decodes it into raw audio data that can be used within our system.
...
- the [`decodeAudioDataSource`](/core/base-audio-context#decodeaudiodatasource) method can be used to load audio data from a local audio source in the form of an [`AudioBuffer`](sources/audio-buffer).
