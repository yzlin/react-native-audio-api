...
`GainNode` is a simple node that can change the volume of any node (or nodes) connected to it. The `GainNode` has a single element called [`AudioParam`](/docs/core/audio-param) , which is also named `gain`..., a bit lame, but we have to live with that.
...
Please read [`ADSR and envelope`](/docs/effects/gain-node#envelope---adsr) section to become familiar with common sound manipulation techniques.
...
- [`AudioBufferSourceNode`](/docs/sources/audio-buffer-source-node) has a parameter called `playbackRate` that allows you to change the speed of the audio buffer's playback, thereby altering the pitch of the sound.
