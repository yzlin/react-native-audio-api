...
| Parameters | Type | Description |
| :---: | :---: | :---- |
| `pitchCorrection` <Optional /> | [`AudioBufferBaseSourceNodeOptions`](/docs/core/base-audio-context#audiobufferbasesourcenodeoptions) | Dictionary object that specifies if pitch correction has to be available. |
...
`AudioBufferBaseSourceNodeOptions` is a dictionary object specifies if pitch correction algorithm has to be available.
