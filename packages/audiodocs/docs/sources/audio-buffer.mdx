...
It can be created from audio file using [`BaseAudioContext.decodeAudioDataSource`](/core/base-audio-context#decodeaudiodatasource), [`BaseAudioContext.decodeAudioData`](/core/base-audio-context#decodeaudiodata)  or from raw data using [constructor](/sources/audio-buffer#constructor).
...
Above method returns modifiable array with PCM data from given channel.
