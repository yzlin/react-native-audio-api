...
[`BaseAudioContext.createBufferSource()`](/core/base-audio-context#createbuffersource)
...
| `timeStretch` | [`TimeStretchType`](/types/time-stretch-type) | Enumerated value that specifies the algorithms used when performing time-stretching and pitch-shifting |

## Methods

### `start` <Overridden />

Above method schedules `AudioBufferSourceNode` to start playback of audio data contained in the associated `AudioBuffer`, or starts to play immediately.
...
- When `AudioBufferSourceNode.timeStretch = 'speech-music'` it is clamped to range -1200 to 1200.
...
- When `AudioBufferSourceNode.timeStretch = 'speech-music'` it is clamped to range 0 to 3.
