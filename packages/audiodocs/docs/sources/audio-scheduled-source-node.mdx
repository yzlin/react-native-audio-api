...
Above method schedules the node to start audio playback at specified time, if no time is given, it starts immediately.
...
Above method schedules the node to stop audio playback at specified time, if no time is given, it stops immediately.
